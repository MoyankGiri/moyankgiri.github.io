...
|---|---|---|
| `_data/site.yml` | nav, hero, footer, contact | name, tagline, contact info, social links, résumé PDF path, hero subtitle, About paragraph, nav links (`{label, href}`, plus an optional `tab:` on any link pointing at `#journey` — see Nav below) |
...
| `_data/companies.yml` | hero logo marquee | `{name, logo}` list of companies/institutions shown as a grayscale auto-scrolling strip in the hero's left column — `logo` is a path under `assets/img/` |
| `_data/skills.yml` | `#skills` | skill groups (`category` + `items[]`, each item `{name, icon, months}` — `icon` is an optional [Simple Icons](https://simpleicons.org) slug rendered via `cdn.simpleicons.org`, `""` for concepts with no real brand mark; `months` is total hands-on experience in months, sourced from the user's own skills-vs-projects tracker — `months: 0` means "no quantified figure," which renders the skill as a plain chip instead of a bar) |
...
| `_data/timeline.yml` | `#journey` Gantt strip | `span_start`/`span_end` (`YYYY-MM`, or `"present"`) plus `lanes[]` (`{lane, segments[]}`, each segment `{label, start, end}`) — a compact 2-lane (Academics/Experience) career overview rendered above the tabs; `assets/js/main.js` converts the YYYY-MM strings to month-offset percentages at runtime, no date math in Liquid |
...
- `assets/js/main.js` — vanilla JS: mobile nav toggle, `IntersectionObserver`-based scroll-spy (special-cased for `#journey` — since Experience/Education/Achievements nav links all point there, it highlights whichever nav link matches the *currently active tab*, not all three at once), a named `activateTab(id)` function shared by the tab buttons and by any nav link carrying `data-tab` (so "Education"/"Achievements" in the nav both scroll to `#journey` and switch to the right tab), a generic "see more" expand/collapse handler (`.see-more-link`, keyed off `.is-extra` items in the element right before the button), the Projects gallery controller (search input, sort toggle, grid/list view toggle, and a Category/Tech-Stack filter sidebar built at runtime from each card's `data-category`/`data-skills` attributes), and `initGantt()` (reads `#gantt-chart`'s segments' `data-start`/`data-end` and positions them as inline `left`/`width` percentages against the container's overall span). No animation/motion framework.
...
- **Hero/About are a second deliberate, user-requested deviation from DESIGN.md's literal spec.** DESIGN.md's own text puts the portrait in About and no avatar in Hero; the user explicitly asked for the opposite — the avatar (plus a grayscale auto-scrolling company-logo marquee) lives in the Hero's right column now, and About's right column instead holds a decorative animated "word cloud" of skill/domain terms (`.word-cloud`/`.cloud-word` in `main.css`, markup hand-authored in `about.html` — no data file, since it's purely illustrative and not a fact needing to stay in sync with anything).
- **The Skills bar chart and Journey Gantt strip are sourced from the user's own skills-vs-projects spreadsheet**, not invented figures — see the `months` field on `_data/skills.yml` and the whole of `_data/timeline.yml`. When updating either, prefer the same sourcing discipline: real, user-provided numbers, and `months: 0` (rendered as a plain chip) rather than a guessed figure for anything unquantified.
